...
function audiology_socials($content){
	...
	switch($post->post_name) {
		...
		case 'ipurepatient-twitter':
			$content .= '<div id="twitterBtn"><a href="https://twitter.com/#!/ipurepatient"  class="twitter-follow-button" data-show-count="false" data-size="large">Follow @ipurePatient</a></div>
			<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>
			<img src="'.get_bloginfo('stylesheet_directory').'/images/ipTwitter.png" class="twitterBird" />
			<ul id="twitter_update_list"> 
				<li>Twitter feed loading...</li>
		...
		case 'google':
			if(!empty($options['googleplus'])) {
				...
				$client->setClientId('777155236486.apps.googleusercontent.com');
				$client->setClientSecret('dXeBMVO-q8-xluWdj4fATi9c');
				$client->setRedirectUri('http://www.fuelaudiology.com/oauth2callback');
			    ...
			    $activities = $plus->activities->listActivities($id, 'public', $optParams);
			  	...
			  	if(isset($me) && isset($activities)):
				    $content .= '<div class="me">';
					$content .= '<div><img src="'. substr($me['image']['url'],0, -2) .'82" /></div>';
				    $content .= '<p><a rel="me" href="'. $me['url'] .'">'. $me['displayName'] .'</a></p>';
				    $content .= '<p>'.$me['tagline'].'</p> ';
				    $content .= '<p>'. $me['aboutMe'].'</p>';
				    $content .= '</div>';
				    $content .= '<div class="activities">Your Activities:';
				    foreach($activities['items'] as $activity):
					    $content .= '<div class="activity">';
					    $content .= '<p><a href="'. $activity['url'] .'">'. $activity['title'] .'</a></p>';
					    $content .= '</div>';
				    endforeach;
				    $content .= '</div>';
			    endif;
